    ...
    def __getitem__(self, index):
        ...
        for i, id in enumerate(view_ids):
            image_filename = os.path.join(
                self.img_dir,
                "{}_train/rect_{:0>3}_{}_r5000.png".format(scan, id + 1, light_idx),
            )
            imgs.append(self.read_image(image_filename))
            proj_mat_filename = os.path.join(
                self.camera_dir, "train/{:0>8}_cam.txt"
            ).format(id)
            intrinsic, extrinsic, depth_min, depth_interval = self.read_cam_file(
                proj_mat_filename
            )
            ...
            if i == 0:  # For reference image only
                depth_values = np.arange(
                    depth_min, depth_interval * self.ndepths + depth_min, depth_interval
                )
                maskname = os.path.join(
                    self.depth_dir,
                    scan + "_train",
                    "depth_visual_{:0>4}.png".format(id),
                )
                mask = cv2.imread(maskname, cv2.IMREAD_GRAYSCALE) / 255
                mask = mask > 0.5
                ref_depth_filename = os.path.join(
                    self.depth_dir, scan + "_train", "depth_map_{:0>4}.pfm".format(id)
                )
        ...
        return {
            ...
            "intrinsics": np.stack(
                intrinsics, 0
            ),  # contains 1 ref image and N source images
            "extrinsics": np.stack(
                extrinsics, 0
            ),  # contains 1 ref image and N source images
